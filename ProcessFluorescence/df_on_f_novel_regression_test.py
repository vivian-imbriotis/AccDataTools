...
try:
    ...
    with open(r"C:/Users/viviani/desktop/cache.pkl","wb") as file:
        rec = Recording(r"H:\Local_Repository\CFEB013\2016-05-31_02_CFEB013")
        pkl.dump(rec,file)

loading_finished = time()
...
def plot_loss(i):
    ...
    X, Y = np.meshgrid(X, Y)
    Z = np.vectorize(lambda x,y:underline_cost_func((x,y),rec.Fneu[i],rec.F[i]))(X,Y)
    fig = plt.figure()
    ...
    U,V = np.vectorize(lambda x,y:underline_cost_gradient(
...
def loss(params, x, y):
    guess = params[0] + params[1] * x
    residuals = y - guess #data points above line are positive
    positive_residuals_cost = np.sum(residuals*(residuals>0))
    negative_residuals_cost = np.sum(-K*residuals*(residuals < 0))
    negative_slope_cost = -K*params[1]*(params[1]<0)
    print((positive_residuals_cost,negative_residuals_cost,negative_slope_cost))
    cost = positive_residuals_cost + negative_residuals_cost + negative_slope_cost
    return cost

def grad(params, x, y, diracs = False):
    guess = params[0] + params[1] * x
    residuals = y - guess #data points above line are positive
    theta_0 = -np.sum(1.0*(residuals>0) + K*(residuals<0))
    theta_1 = -np.sum(x*(residuals>0) + x*K*(residuals<0)) + K*(params[0]<0)
...
def plot_clever_loss(i):
    ...
    Y = np.arange(-5, 5, 0.25)
    X, Y = np.meshgrid(X, Y)
    Z = np.vectorize(lambda x,y:loss((x,y),rec.Fneu[i],rec.F[i]))(X,Y)
    fig = plt.figure()
    ax = fig.gca(projection='3d')
    ...
    U,V = np.vectorize(lambda x,y:grad(
        (x,y),rec.Fneu[i],rec.F[i],diracs = False))(X,Y)
...
def underline_cost_gradient(params, x, y, diracs = False):
    guess = params[0] + params[1] * x
    residuals = y - guess #data points above line are positive
    if diracs:
...
def underline_regression(x, y, method = "Powell"):
    ...
    if method not in ("cg","CG", "grad desc", "gradient descent"):
...
class UnderlineRegressionFigure:
    ...
    def __init__(self, recording, roi_number, method = 'robust'):
        ...
        elif method in ("cg","CG","grad descent", "gradient descent"):
...
plot_clever_loss(6)
UnderlineRegressionFigure(rec, 4, method="CG").show()
