...
def quadratic_loss(params,x = rec.Fneu[4], y = rec.F[4],k=4,v=False):
    ...
    lower_cost = np.sum(k*ramp(-residuals)*(2))
    cost = upper_cost + lower_cost
    return cost
...
def plot_loss_functions():
    ...
    ax[0].set_title("Heaviside/L1 loss")
    ax[1].plot(X,quad_loss)
    ax[1].set_title("Asymmetric L2 loss")
    ax[2].plot(X,hubel_loss)
    ax[2].set_title("L2/Pseudo-Huber loss")
    for a in ax:
        a.set_ylabel("Loss")
...
def plot_huber_loss():
    ...
    params = (0,0)
    hubel_loss = [HubelRegressor.loss(params,np.array(0),np.array(x),k=10, delta=0.5) for x in X]
    ax.plot(X,hubel_loss)
    ax.set_ylabel("Loss")
    ax.set_xlabel("Residual ($\hat(F) -F$)")
    ax.set_ylim((0,6))
    fig.show()
